...
const formatCallbackUrl = ({ clientId, redirectUri, scopes }) => {
...
const generateToken = async ({
  ...
  const encodedAuthToken = Buffer.from(`${clientId}:${clientSecret}`).toString(
    "base64"
  );
...
inquirer
  .prompt([
    ...
    },
    {
      ...
      when: async (answers) => {
        ...
        const callbackUrl = formatCallbackUrl({
          clientId,
          redirectUri,
  ...
  .then(async (answers) => {
    ...
    spinner.succeed(
      chalk.green("refresh_token has been copied to the clipboard")
    );
  })
  .catch((error) => {
    if (error.isTtyError) {
